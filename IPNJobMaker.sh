...
WHOAMI=$(whoami)
GRID_FTPSERVER="ipngrid04.in2p3.fr"
pwd=$(pwd)
...
#	echo "OutputSandbox = {\"file.stdout\",\"file.stderr\"};" >> $jdlfile
	  ...
	  jobid=`glite-ce-job-submit -D proxy_martraire  -r ipngrid04.in2p3.fr:8443/cream-pbs-ipno -o jid $jdlfile `
    ...
    joblist_tmp=tmp_l
    rm -f $joblist
    glite-ce-job-list ipngrid04.in2p3.fr:8443>$statusfile 2>/dev/null
  ...
  #lcg-cp -v lfn:$/nfs1/scratch/corsika/Sim_corsika_${ener}_${angle}_${iter}.root file://`pwd`/LhaRootFer/Sim_corsika_${ener}_${angle}_${iter}.root

	  lcg-cp -v /nfs1/scratch/corsika/Sim_corsika_${ener}_${angle}_${iter}.root file://`pwd`/LhaRootGammaRE/Sim_corsika_${ener}_${angle}_${iter}.root
    ...
    joblist_tmp=tmp_l
    rm -f $joblist_tmp
    glite-ce-job-list ipngrid04.in2p3.fr:8443>$statusfile
    while read it e a j
      do 
...
#	  uberftp ${GRID_FTPSERVER} "rm /sandboxes/tmp/$WHOAMI/$jobdir/*.*"   
    ...
    joblist_tmp=tmp_l
    rm -f $joblist
    glite-ce-job-list ipngrid04.in2p3.fr:8443>$joblist_tmp
    ...
    glite-ce-delegate-proxy -e ipngrid04 proxy_martraire
    #glite-ce-proxy-renew -e ipngrid04 proxy_martraire
else 
