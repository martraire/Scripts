...
system($test); 
    ...
    #my $outputdir = $localdir . "/" . $count;
    my $outputdir =  $count;
    mkdir "$outputdir", 0777 unless -d "$outputdir";

    my @expr = ( "!name!" );
    my $inputtobecopied = "$localdir/EventFileReader.temp.xml";
    open IN, $inputtobecopied or die "Can't read source file $inputtobecopied: $!\n";

    my $input = $outputdir . "/EventFileReader.xml";
    open OUT, ">$input" or die "Can't write on file $input: $!\n";
    
    my $name = $outputdir . "/" . $name_file[$count] . ".part";
    ...
    my $truc = "ln -s " . $inputdir . "/EventChecker.xml " . $outputdir . "/EventChecker.xml ";
    system($truc);
    $truc = "ln -s " . $inputdir . "/EventGenerator.xml " . $outputdir . "/EventGenerator.xml ";
    system($truc);
    $truc = "ln -s " . $inputdir . "/ModuleSequence.xml " . $outputdir . "/ModuleSequence.xml ";
    system($truc);
    $truc = "ln -s " . $inputdir . "/SManagerRegisterConfig.xml " . $outputdir . "/SManagerRegisterConfig.xml ";
    system($truc);
    $truc = "ln -s " . $inputdir . "/SdCalibrator.xml " . $outputdir . "/SdCalibrator.xml ";
    system($truc);
    $truc = "ln -s " . $inputdir . "/bootstrap.xml " . $outputdir . "/bootstrap.xml ";
    system($truc);
    $truc = "ln -s " . $inputdir . "/userAugerOffline " . $outputdir . "/userAugerOffline ";
    system($truc);
    ...
    chdir $outputdir;
    my $command = "iget " . $path[$count] . " . ; ";
    system($command);
    $command = "./userAugerOffline -b bootstrap.xml >& useAugerOffline.log" ;
    system($command);
    $command = "rm -f " . $outputdir . "/" .$name_file[$count] . ".part" ;
    system($command);
    $command = "mv " . $outputdir . "/ascii.txt " . $inputdir . "/Ascii_files/ascii_" . $count . ".txt" ;
    system($command);
